...
Thriller::Thriller(char* name, int  outYear, int level) :Movies(level), Store(name, outYear)//ctor
{
	levelSelect(level);
}
bool Thriller::levelSelect(int level)
{

	while (this->level < 1 || this->level>6)
	{
...
void Thriller::toOs(ostream& os) const//prints the additional information of this product type
{
	os << name << "> <" << outYear << "> <" << level << ">" << endl;

}
Thriller::Thriller(const Thriller& other) //copy ctor
{
